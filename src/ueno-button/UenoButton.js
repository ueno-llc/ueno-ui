...
import { ArrowRight, ArrowSubmit } from '../icons/Icons';
...
export default class UenoButton extends Component {
  ...
  onMouseEnter = ({ force = false } = {}) => {
    ...
    t
      .fromTo(
        ...
        duration,
        { x: '-103%' },
        { x: '-30%', ease: window.Power4.easeInOut },
      )
      .fromTo(
        arrow,
        duration,
        { x: '0' },
        { x: '-15px', ease: window.Power4.easeInOut },
        '-=0.8',
      )
      .fromTo(
        text,
        duration,
        { x: '0' },
        { x: '20px', ease: window.Power4.easeInOut },
        '-=0.8',
      );
  };
  ...
  onMouseLeave = ({ force = false } = {}) => {
    ...
    t
      .to(hover, duration, { x: '103%', ease: window.Power4.easeInOut })
      .to(arrow, duration, { x: '0', ease: window.Power4.easeInOut }, '-=0.8')
      .to(text, duration, { x: '0', ease: window.Power4.easeInOut }, '-=0.8');
  };

  renderContent() {
    const { submit, arrowBack, children } = this.props;
    if (submit) {
      return (
